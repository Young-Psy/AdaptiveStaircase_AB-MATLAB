function [] = AB_staircase_post_interval_rhythm_2408()
...
destdir = './interval&rhythm/posttest/';
...
%             ISIseq(T1realloc+T2realloc-1)  = expinfo.stdura*(T2pos-T2realloc)+ numISI;%the position of T2
            ...
            if resp(1)-lettlist_perm(T1pos) == 0 %判断T1回答
                if resp(2) - lettlist_perm(T1pos+T2pos) == 0 %判断T2回答
                    acc = 1;
                else
                    acc = 0;
                end
            ...
            for i = 1:2
                if trldouble(blk) == i
                    ...
                    data.lett{i,blk}(trl(i,blk),:)        = [lettlist_perm(T1pos) lettlist_perm(T1pos+T2pos)];
...
%                         if sum(rowofoutput{i,blk}(trl(i,blk)-expinfo.outtrials+1:trl(i,blk),2)) == expinfo.steprange(i)*expinfo.outtrials
        ...
        for i = 1:2
            data.thresholds{i,blk}(:,:)   = expthresholds{i,blk};  % !!!数据保存
    ...
    %   画图 上面一行都是从300ms开始的，下面一行都是从1000ms开始的，每个blk的图上下对应，方便看
    figure;
    plotNum1 = 1;
    plotNum2 = 4;
